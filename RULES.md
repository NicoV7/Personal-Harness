RULES - These are a set of rules for the agent to follow

1. Search the brain AND the web BEFORE you do ANYTHING substantive.
Before proposing a plan, before debugging, before recommending tools.

2. Before compaction, save all context to the database.
...
6. Ask about the requirements
    Before implementing we need to consider scalability, maintainability, and reliability
    When designing architecture, it is best to layer the architecture in service, models, dtos, tests, tools, constants, handlers
    Avoid GOD files - over 2000 lines of code should be heavily considered for a refactor. 

7. Avoid Overly Defensive Programming
    Suppressed Bugs: Hiding exceptions prevents logs from capturing errors, leaving developers blind to issues until they corrupt downstream data.
    Code Bloat: Writing three lines of logic wrapped in ten lines of safety boilerplate hurts code readability and maintainability.
    Unpredictable States: Returning fallback values (like an empty object or string) makes the program continue running in a "zombie" state with bad data.
    Circular Imports: Reduce maintainability, imports belong at the top of the file.

=================================================================================================================================================================

The follow points are examples related to the rules stated aboved
...
4. Overly Definsive Programming AVOID THESE

4.1 Catch-All Exception Masking ("Swallowing" Errors). A developer uses a broad try-catch block to handle an operation but leaves the catch segment empty or returns a generic fallback value. This ensures the application won't crash, but it fundamentally hides broken logic.The Overly Defensive Way (JavaScript):
 
 javascriptfunction getUserDashboard(userId) {
  try {
    ...
    The Better Way: Let the error bubble up naturally to a global error tracker or logging boundary, or catch only specific, actionable exceptions.
...
4.2 Deep Optional Chaining Hell
    Optional chaining (?.) or deep boolean shortcuts are frequently used to traverse nested objects when the contract of the data structure should already guarantee its shape.The Overly Defensive Way (JavaScript):

    javascript// Defending against data that shouldn't be missing internally
    const title = response?.data?.user?.profile?.settings?.theme?.title || "Default";

    The Better Way: Enforce data validation at the API or service boundary (using a validator like Zod) so internal components can rely on a fixed schema.

    javascriptconst title = response.data.user.profile.settings.theme.title;

4.3 Redundant Internal Type and Null Checking

Checking types and variables within internal, private functions where the inputs have already been validated by the parent method or typescript compiler.The Overly Defensive Way (Python):
pythondef _calculate_internal_bonus(salary):
...
The Better Way: Use type hinting and let the function crash fast and loud if it somehow receives an illegal type.pythondef _calculate_internal_bonus(salary: float) -> float:
    return salary * 0.10
