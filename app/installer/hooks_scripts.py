...
# the client session. ALWAYS exits 0 — deny/block decisions travel in
# the printed JSON, and the client only processes hook JSON at exit 0.
...
if [ "$CURL_EXIT" -ne 0 ]; then
  echo "$(date -u +%FT%TZ) {name} curl_exit=$CURL_EXIT" >> "$BETTERAI_HOME/hook-errors.log"
fi
if [ -n "$RESPONSE" ]; then
  printf '%s\\n' "$RESPONSE"
fi
